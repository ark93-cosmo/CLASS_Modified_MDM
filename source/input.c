...
/** what's the tmp file for? temporary file*/
  int i;
  char extension[5];
  ...
  if (argc > 1) {
    for (i=1; i<argc; i++) {
      strncpy(extension,(argv[i]+strlen(argv[i])-4),4);/** strnpcpy copies from argv to extension*/ 
      extension[4]='\0';
      if (strcmp(extension,".ini") == 0) {
        class_test(input_file[0] != '\0',/** class_test() offers a generic way to return an error in a standard format if a condition is not fulfilled*/
                   errmsg,
  ...
  /** If the extension is of .ini type, then store the content of the file with the name argv into the input file. On the other hand, if the extension 
		is of .pre type, store the content in the precision file. If neither, use default values. */

  /** - if there is an 'xxx.ini' file, read it and store its content. */

  if (input_file[0] != '\0'){

    class_call(parser_read_file(input_file,&fc_input,errmsg),/** a parser is a program that analysis a string into its logical components*/
               errmsg,
               errmsg);

    /** - check whether a root name has been set *//** what's the use of the root? the root is the path to a specific file in the output folder*/
  ...
  class_call(input_init(&fc,
                        ppr,/*pointer for precision*/
                        pba,/*pointer for background*/
                        pth,
                        ppt,
                        ptr,
                        ppm,
...
int input_read_parameters(
  ...
  class_call(parser_read_double(pfc,"analytic_mimetic_percent",&param1,&flag1,errmsg),
             errmsg,
             errmsg);
     if (flag1 == _TRUE_){
		 ppt->analytic_mimetic_percent = param1;
		 }
		 class_call(parser_read_double(pfc,"rand_mimetic_percent",&param1,&flag1,errmsg),
             errmsg,
             errmsg);
     if (flag1 == _TRUE_){
		 ppt->rand_mimetic_percent = param1;
		 }
  ...
  /** -Omega_0_mimetic */
  class_call(parser_read_double(pfc,"Omega_mimetic",&param3,&flag3,errmsg),errmsg,errmsg);
  class_test(((flag3 == _TRUE_) && ((flag1 == _TRUE_) || (flag2 == _TRUE_))),errmsg, "In input file, you can only enter CDM or mimetic");
  if (flag3 == _TRUE_){
	pba->Omega0_mimetic = param3;
	pba->Omega0_cdm = 0.0;
	Omega_tot += pba->Omega0_mimetic;
  }
  /** - Omega_0_dcdmdr (DCDM) */
...
int input_default_params(
  ...
  pba->Omega0_cdm = 0.0/*0.12038/pow(pba->h,2)*/;
  pba->Omega0_mimetic = 0.0;
  ppt->analytic_mimetic_percent = 0.0;
  ppt->rand_mimetic_percent = 0.0;
  ...
  pba->K = 0.;
  pba->sgnK = 0;
  pba->Omega0_lambda = 1.-pba->Omega0_k-pba->Omega0_g-pba->Omega0_ur-pba->Omega0_b-pba->Omega0_cdm-pba->Omega0_mimetic-pba->Omega0_ncdm_tot-pba->Omega0_dcdmdr;
  pba->Omega0_fld = 0.;
  pba->a_today = 1.;
  pba->use_ppf = _TRUE_;
