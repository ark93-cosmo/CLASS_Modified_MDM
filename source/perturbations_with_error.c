...
int perturb_init(
  ...
  class_test((ppt->gauge == synchronous) && (pba->has_mimetic == _TRUE_),
             ppt->error_message,
             "You can't study mimetic dark matter in the synchronous gauge!");
...
int perturb_vector_init(
  ...
  if (_scalars_) {
    ...
    class_define_index(ppv->index_pt_delta_mimetic,pba->has_mimetic,index_pt,1); /* mimetic density */
    class_define_index(ppv->index_pt_theta_mimetic,pba->has_mimetic,index_pt,1); /* mimetic velocity */
...
int perturb_initial_conditions(struct precision * ppr,
  ...
  int min = -1;
  int step = 2;
  int r = (rand()%((++max-min)/step))*step+min;//random for theta_mimetic 
  ...
  if (_scalars_) {
    ...
    */
    ...
    if ((ppt->has_ad == _TRUE_) && (index_ic == ppt->index_ic_ad)) {
      ...
      if (pba->has_mimetic == _TRUE_) {
        ppw->pv->y[ppw->pv->index_pt_delta_mimetic] = 3./4.*ppw->pv->y[ppw->pv->index_pt_delta_g]; /* mimetic density */
        /* mimetic velocity would vanish in something comparable to the synchronous gauge */
      }
    ...
    if (ppt->gauge == newtonian) {
      ...
      */
      ...
      if (pba->has_cdm == _TRUE_)
        ...
        delta_cdm = ppw->pv->y[ppw->pv->index_pt_delta_mimetic];
	  ...
	  /** HERE critical point -> delta mimetic **/
      ...
      if (pba->has_mimetic == _TRUE_) {
		ppw->pv->y[ppw->pv->index_pt_delta_mimetic] -= 3.*a_prime_over_a*alpha;
		if (ppt->analytic_mimetic_percent != 0.0){
        ppw->pv->y[ppw->pv->index_pt_theta_mimetic] = k*k*alpha*(1+sin(log(k)/log(2))*ppt->analytic_mimetic_percent);
	     }
	     if (ppt->rand_mimetic_percent != 0.0){
			 ppw->pv->y[ppw->pv->index_pt_theta_mimetic] = k*k*alpha*(1+r2*ppt->rand_mimetic_percent);
			 }
      }
...
int perturb_total_stress_energy(
  ...
  if (_scalars_) {
    ...
    /* mimetic dark matter contribution */
    if (pba->has_mimetic == _TRUE_) {
      ppw->delta_rho += ppw->pvecback[pba->index_bg_rho_mimetic]*y[ppw->pv->index_pt_delta_mimetic];
    ...
    */
    ...
    if (ppt->has_source_delta_m == _TRUE_) {
      ...
      /* include mimetic dark matter */
      if (pba->has_mimetic == _TRUE_) {
        delta_rho_m += ppw->pvecback[pba->index_bg_rho_mimetic]*y[ppw->pv->index_pt_delta_mimetic];
        rho_m += ppw->pvecback[pba->index_bg_rho_mimetic];
      }
...
int perturb_sources(
  ...
  if (_scalars_) {
    ...
    /* delta_mimetic */
    if (ppt->has_source_delta_mimetic == _TRUE_) {
    ...
    /* theta_mimetic */
    if (ppt->has_source_theta_mimetic == _TRUE_) {
...
int perturb_derivs(double tau,
  ...
  if (_scalars_) {
    ...
    if (pba->has_mimetic == _TRUE_) {
      ...
      dy[pv->index_pt_delta_mimetic] = -(y[pv->index_pt_theta_mimetic]+metric_continuity); /* mimetic density */

      dy[pv->index_pt_theta_mimetic] = - a_prime_over_a*y[pv->index_pt_theta_mimetic] + metric_euler; /* mimetic velocity */
