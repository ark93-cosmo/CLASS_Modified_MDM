...
int background_functions(
  ...
  if (pba->has_mimetic == _TRUE_) {
	  //has_cdm == _FALSE_;
    pvecback[pba->index_bg_rho_mimetic] = pba->Omega0_mimetic * pow(pba->H0,2) / pow(a_rel,3);
    //printf("rho = %f",pvecback[pba->index_bg_rho_mimetic]);
    rho_tot += pvecback[pba->index_bg_rho_mimetic];
    p_tot += 0.;
    rho_m += pvecback[pba->index_bg_rho_mimetic];
  }
  //else {pvecback[pba->index_bg_rho_mimetic] = 0.0;}
...
int background_w_fld(
  ...
  case EDE:
    ...
    else if (pba->has_mimetic == _TRUE_){Omega_m += pba->Omega0_mimetic;}
    if (pba->has_dcdm == _TRUE_)
...
int background_indices(
  ...
  if (pba->Omega0_mimetic != 0.)
    pba->has_mimetic = _TRUE_;
  ...
  /* - index for rho_mimetic*/
  class_define_index(pba->index_bg_rho_mimetic,pba->has_mimetic,index_bg,1);
...
int background_output_titles(struct background * pba,
  ...
  class_store_columntitle(titles,"(.)rho_mimetic",pba->has_mimetic);
  if (pba->has_ncdm == _TRUE_){
...
int background_output_data(
  ...
  for (index_tau=0; index_tau<pba->bt_size; index_tau++){
    ...
    class_store_double(dataptr,pvecback[pba->index_bg_rho_mimetic],pba->has_mimetic,storeidx);
    if (pba->has_ncdm == _TRUE_){
      for (n=0; n<pba->N_ncdm; n++){
...
int background_derivs(
  ...
  if (pba->has_cdm){
    rho_M += pvecback[pba->index_bg_rho_cdm];}
  //else if (pba->has_mimetic){rho_M += pvecback[pba->index_bg_rho_mimetic];}
