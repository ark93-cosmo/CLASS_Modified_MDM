...
int perturb_indices_of_perturbs(
  ...
  ppt->has_source_delta_mimetic = _FALSE_;
  ppt->has_source_theta_mimetic = _FALSE_;
  ...
  for (index_md = 0; index_md < ppt->md_size; index_md++) {
    ...
    if (_scalars_) {
      ...
      if (ppt->has_density_transfers == _TRUE_) {
        ...
        if (pba->has_mimetic == _TRUE_){
          ppt->has_source_delta_mimetic = _TRUE_;
          }
        if (pba->has_ur == _TRUE_)
      ...
      if (ppt->has_velocity_transfers == _TRUE_) {
        ...
        if (pba->has_mimetic == _TRUE_){
          ppt->has_source_theta_mimetic = _TRUE_;
        }
        if (pba->has_ur == _TRUE_)
      ...
      class_define_index(ppt->index_tp_delta_mimetic,  ppt->has_source_delta_mimetic, index_type,1);
      ...
      class_define_index(ppt->index_tp_theta_mimetic,  ppt->has_source_theta_mimetic, index_type,1);
...
int perturb_prepare_output(struct background * pba,
  ...
  if (ppt->k_output_values_num > 0) {
    ...
    if (ppt->has_scalars == _TRUE_){
      ...
      /*Mimetic Field*/
       class_store_columntitle(ppt->scalar_titles,"theta_mimetic",pba->has_mimetic);
      ...
      /* Mimetic Field mimetic */
      class_store_columntitle(ppt->scalar_titles, "theta_mimetic", pba->has_mimetic);
      class_store_columntitle(ppt->scalar_titles, "delta_mimetic", pba->has_mimetic);
...
int perturb_vector_init(
  ...
  if (_scalars_) {
    ...
    class_define_index(ppv->index_pt_delta_mimetic,pba->has_mimetic,index_pt,1); /* mimetic field density perturbation, equivalent to g(x,tau)/a^2\bar{\rho}_{\varphi} in paper */
    class_define_index(ppv->index_pt_theta_mimetic,pba->has_mimetic,index_pt,1); /* mimetic field "velocity", v(\varphi) in paper*/
  ...
  /** - case of switching approximation while a wavenumber is being integrated */
    ...
    if (_scalars_) {
      ...
      if (pba->has_mimetic == _TRUE_){
		 ppv->y[ppv->index_pt_delta_mimetic] =
          ppw->pv->y[ppw->pv->index_pt_delta_mimetic]; 
		 ppv->y[ppv->index_pt_theta_mimetic] =
            ppw->pv->y[ppw->pv->index_pt_theta_mimetic];
		 }
      ...
      if (pba->has_mimetic == _TRUE_) {
        ...
        /*ppv->y[ppv->index_pt_delta_phi_mimetic] =
          ppw->pv->y[ppw->pv->index_pt_delta_phi_mimetic];*/
...
int perturb_initial_conditions(struct precision * ppr,
  ...
  double f_dr;
  double theta_mimetic;
  double delta_mimetic;
  double delta_tot;
  double velocity_tot;
  double s2_squared;
  ...
  if (_scalars_) {
    ...
    if(pba->has_mimetic == _TRUE_){
		rho_m +=ppw->pvecback[pba->index_bg_rho_mimetic];
		}
    ...
    */
    ...
    if ((ppt->has_ad == _TRUE_) && (index_ic == ppt->index_ic_ad)) {
      ...
      /*if (pba->has_mimetic == _TRUE_){
		  has_cdm == _FALSE_;
		  ppw->pv->y[ppw->pv->index_pt_theta_mimetic]=0.0;
		  }*/
    ...
    if (ppt->gauge == newtonian) {
      ...
      */
      ...
      if (pba->has_cdm == _TRUE_)
        ...
        delta_cdm=0.;
	  /*if ((pba->has_mimetic == _TRUE_) && (ppt->has_ad == _TRUE_) ){
		  ppw->pv->y[ppw->pv->index_pt_theta_mimetic]= ppt->theta_mimetic0;
		  ppw->pv->y[ppw->pv->index_pt_delta_mimetic] = -((3.*a_prime_over_a*a_prime_over_a-a*a*ppw->pvecback[pba->index_bg_rho_mimetic])*(delta_tot+2*ppw->pvecmetric[ppw->index_mt_psi])+6*a_prime_over_a*ppw->index_mt_phi_prime+2*k2*ppw->pv->index_pt_phi)/(a*a*ppw->pvecback[pba->index_bg_rho_mimetic]);
		  }*/
		  /*equation 3.21 in paper*/
	  ...
	  if (pba->has_mimetic == _TRUE_){ 
		 ppw->pv->y[ppw->pv->index_pt_delta_mimetic] =3./4.*ppw->pv->y[ppw->pv->index_pt_delta_g]; /*-((3.*a_prime_over_a*a_prime_over_a-a*a*ppw->pvecback[pba->index_bg_rho_mimetic])*(delta_tot+2*ppw->pvecmetric[ppw->index_mt_psi])+6*a_prime_over_a*ppw->index_mt_phi_prime+2*k2*ppw->pv->index_pt_phi)/(a*a*ppw->pvecback[pba->index_bg_rho_mimetic]);*/
		 
		  delta_tot += rho_m_over_rho_r*fraccdm*ppw->pv->y[ppw->pv->index_pt_delta_mimetic]/(1.+rho_m_over_rho_r);
		  
		  ppw->pv->y[ppw->pv->index_pt_theta_mimetic]= k*k*alpha;//ppt->theta_mimetic0;
		  
		  velocity_tot += (4./3.*rho_m_over_rho_r*fraccdm*ppw->pv->index_pt_theta_mimetic)/(1.+rho_m_over_rho_r);
  ...
  if (_scalars_) {
    ...
    if (ppt->gauge == newtonian) {
      ...
      /* in principle we could get phi from the constrain equation:
         ...
         ppw->pvecmetric[ppw->index_mt_phi] = -1.5 * (a2/k2/k2/s2/s2) * (k2 * delta_rho + 3.*a_prime_over_a * rho_plus_p_theta); This is the combination of (23a) and (23b) in Ma& Bertschinger
...
int perturb_total_stress_energy(
  ...
  if (_scalars_) {
    ...
    if (pba->has_mimetic == _TRUE_){
		ppw->rho_plus_p_theta += ppw->pvecback[pba->index_bg_rho_mimetic]*y[ppw->pv->index_pt_theta_mimetic];
		rho_plus_p_tot += ppw->pvecback[pba->index_bg_rho_mimetic];
		}
    ...
    */
    ...
    if ((ppt->has_source_delta_m == _TRUE_) || (ppt->has_source_theta_m == _TRUE_)) {
      ...
      if (pba->has_mimetic == _TRUE_){
		  rho_plus_p_theta_m += ppw->pvecback[pba->index_bg_rho_mimetic]*y[ppw->pv->index_pt_theta_mimetic];
        rho_plus_p_m += ppw->pvecback[pba->index_bg_rho_mimetic];
		  }
...
int perturb_sources(
  ...
  if (_scalars_) {
    ...
    /*else if (ppt->has_source_delta_mimetic == _TRUE_){
		_set_source_(ppt->index_tp_delta_mimetic) = y[ppw->pv->index_pt_delta_mimetic];
		ppw->delta_m += ppw->pv->y[ppw->pv->index_pt_delta_mimetic];
		ppw->delta_cb += ppw->pv->y[ppw->pv->index_pt_delta_mimetic];
		_set_source_(ppt->index_tp_delta_m) += ppw->pv->index_pt_delta_mimetic;
		_set_source_(ppt->index_tp_delta_cb) += ppw->pv->index_pt_delta_mimetic;
		}*/
    ...
    else if (ppt->has_source_theta_mimetic == _TRUE_){
		_set_source_(ppt->index_tp_theta_mimetic) = y[ppw->pv->index_pt_theta_mimetic];
		/*ppw->theta_m += ppw->pv->y[ppw->pv->index_pt_theta_mimetic];
		ppw->theta_cb += ppw->pv->y[ppw->pv->index_pt_theta_mimetic];
		_set_source_(ppt->index_tp_theta_m) += ppw->pv->index_pt_theta_mimetic;
		_set_source_(ppt->index_tp_theta_cb) += ppw->pv->index_pt_theta_mimetic;*/
		}
...
int perturb_print_variables(double tau,
  ...
  double delta_mimetic=0., theta_mimetic=0.;
  ...
  if (_scalars_) {
    ...
    if (pba->has_mimetic == _TRUE_){
		delta_mimetic = y[ppw->pv->index_pt_delta_mimetic];
		theta_mimetic = y[ppw->pv->index_pt_theta_mimetic];
		}
    ...
    /* Mimetic Field */
    class_store_double(dataptr, delta_mimetic, pba->has_mimetic, storeidx);
    class_store_double(dataptr, theta_mimetic, pba->has_mimetic, storeidx);
    /* Non-cold Dark Matter */
    if ((pba->has_ncdm == _TRUE_) && ((ppt->has_density_transfers == _TRUE_) || (ppt->has_velocity_transfers == _TRUE_) || (ppt->has_source_delta_m == _TRUE_))) {
      for(n_ncdm=0; n_ncdm < pba->N_ncdm; n_ncdm++){
...
int perturb_derivs(double tau,
  ...
  if (_scalars_) {
    ...
    else if (pba->has_mimetic == _TRUE_){
		dy[pv->index_pt_delta_mimetic] = -(y[pv->index_pt_theta_mimetic]+metric_continuity);
		dy[pv->index_pt_theta_mimetic] = -a_prime_over_a*y[pv->index_pt_theta_mimetic]+k2*pvecmetric[ppw->index_mt_psi];/*mimetic field velocity*/
		}
